...
particleThread::particleThread(int _rate, const string &_name, const string &_robot,
...
void particleThread::run()
{
    ...
    switch (step)
    {
        ...
        case 2:
            break;
        default:
            yError("[particleThread] particleThread should never be here!!! Step: %d",step);
            yarp::os::Time::delay(2.0);
            break;
    }
}

Vector particleThread::solveIK(int &_exit_code)
{
...
bool particleThread::controlArm(const yarp::sig::Vector &_vels)
{   
...
bool particleThread::stopControl()
{
...
bool particleThread::setTol(const double _tol)
{
    return tol=_tol;
}

bool particleThread::setTrajTime(const double _traj_time)
{
    if (_traj_time>=0.0)
    {
...
bool particleThread::setVerbosity(const int _verbosity)
{
    return _verbosity>=0?verbosity=_verbosity:verbosity=0;
}

bool particleThread::setNewTarget(const Vector& _x_d)
{
    toggleTask(true);
    if (_x_d.size()==3)
    {
        ...
        t_d=t_0+trajTime;
        yInfo("[particleThread] got new target. x_0: %s",x_0.toString(3,3).c_str());
        yInfo("[particleThread]                 x_d: %s",x_d.toString(3,3).c_str());
        yInfo("[particleThread]                 t_d: %g",t_d);
        step = 1;
...
bool particleThread::setNewRelativeTarget(const Vector& _rel_x_d)
{
    Vector x_d = x_t + _rel_x_d;
    return setNewTarget(x_d);
}

void particleThread::updateArmChain()
{
...
bool particleThread::alignJointsBounds()
{
...
bool particleThread::areJointsHealthyAndSet(VectorOf<int> &jointsToSet,
...
bool particleThread::setCtrlModes(const VectorOf<int> &jointsToSet,
                                   const string &_p, const string &_s)
{
